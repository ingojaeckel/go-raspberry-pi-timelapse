...
test.describe('SetupComponent', () => {
  ...
  test('should fetch configuration on mount', async ({ page }) => {
    const requestPromise = page.waitForRequest('**/config');
    ...
    expect(request.url()).toContain('/config');
  });

  test('should display action buttons', async ({ page }) => {
    await page.waitForResponse('**/config');
  ...
  test('should handle save button click', async ({ page }) => {
    ...
    await page.waitForRequest(request => 
      request.url().includes('/config') && request.method() === 'POST'
    );
  });
});
